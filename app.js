...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- 7. A BETTER SOLUTION HOVERING 4-COLOR INTERACTIVE PANEL DECK ---
  const solutionHoverPanel = document.getElementById('solution-hover-panel');
  ...
  const solutionStepPills = document.querySelectorAll('.step-pill');
  const solutionPanelWrapper = document.getElementById('solution-panel-wrapper');
  const solutionsSection = document.getElementById('solutions');

  let currentSolutionStep = 1;
  let isSwishingBack = false;

  function setSolutionStep(stepNum) {
    if (isSwishingBack) return;
    currentSolutionStep = stepNum;

    solutionPanelCards.forEach(card => {
      const cardStep = parseInt(card.getAttribute('data-step'));
      if (cardStep === currentSolutionStep) {
    ...
    solutionStepPills.forEach(pill => {
      const pillStep = parseInt(pill.getAttribute('data-step'));
      if (pillStep === currentSolutionStep) {
  ...
  function advanceSolutionStep() {
    if (isSwishingBack) return;

    if (currentSolutionStep < solutionPanelCards.length) {
      // Fade into next step (1 -> 2 -> 3 -> 4)
      setSolutionStep(currentSolutionStep + 1);
    } else {
      // Reached step 4! Trigger swish-back with edge ruffling effect back to step 1
      isSwishingBack = true;
      if (solutionCardDeck) solutionCardDeck.classList.add('swishing-back');

      setTimeout(() => {
        setSolutionStep(1);
        if (solutionCardDeck) solutionCardDeck.classList.remove('swishing-back');
        isSwishingBack = false;
      }, 880);
    }
  }

  if (solutionHoverPanel) {
    solutionHoverPanel.addEventListener('click', () => {
      advanceSolutionStep();
    });

    solutionHoverPanel.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        advanceSolutionStep();
      }
  ...
  // IntersectionObserver to slide into screen whenever scrolled out and back into section
